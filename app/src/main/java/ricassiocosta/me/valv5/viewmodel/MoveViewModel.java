...
public class MoveViewModel extends ViewModel {
    private static final String TAG = "MoveViewModel";
    ...
    public void start(FragmentActivity activity) {
        ...
        thread = new Thread(() -> {
            ...
            SecureLog.d(TAG, "start: moving " + SecureLog.safeCount(files.size(), "files"));
            DocumentFile destinationDocument = DocumentFile.fromTreeUri(activity, destinationUri);
            for (GalleryFile f : files) {
                boolean success = FileStuff.moveTo(activity, f, destinationDocument);
                if (success) {
                    doneFiles.add(f);
                    onProgress.onProgress(doneFiles.size());
                    FileStuff.deleteFile(activity, f.getUri());
                    FileStuff.deleteFile(activity, f.getThumbUri());
                    FileStuff.deleteFile(activity, f.getNoteUri());
                }
            }
